...
pkg_pretend () {
    ...
    for cat in ${categories[@]}; do
        ...
        if ! has_version -b "${cat}/mingw64-runtime[libraries]" ||
            ...
            einfo "In short:"
            einfo ""
            einfo "emerge -av sys-devel/crossdev"
            einfo "crossdev --stable --libc \"~7.0.0\" --target ${cat}"
            einfo ""
            einfo "echo '${cat}/gcc mingw-gcc.conf' >> /etc/portage/package.env/mingw"
            einfo "echo 'EXTRA_ECONF=\"--enable-threads=posix\"' >> /etc/portage/env/mingw-gcc.conf"
            einfo "echo '${cat}/mingw64-runtime libraries' >> /etc/portage/package.use/mingw"
            einfo ""
            einfo "emerge --oneshot -av ${cat}/mingw64-runtime ${cat}/gcc"
            einfo ""
            einfo ""
