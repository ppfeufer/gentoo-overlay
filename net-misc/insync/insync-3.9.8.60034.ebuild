...
SRC_URI="https://cdn.insynchq.com/builds/linux/${PV}/insync_${PV}-trixie_amd64.deb"
...
src_install() {
	...
	rm -rf "${S}"/opt/insync/PySide2/Qt/qml/Qt/labs/calendar/libqtlabscalendarplugin.so
	rm -rf "${S}"/opt/insync/PySide2/Qt/qml/Qt/labs/platform/libqtlabsplatformplugin.so
	rm -rf "${S}"/opt/insync/PySide2/Qt/qml/QtQuick/Controls.2/Fusion/libqtquickcontrols2fusionstyleplugin.so
	rm -rf "${S}"/opt/insync/PySide2/Qt/qml/QtQuick/Controls.2/Imagine/libqtquickcontrols2imaginestyleplugin.so
	rm -rf "${S}"/opt/insync/PySide2/Qt/qml/QtQuick/Controls.2/Material/libqtquickcontrols2materialstyleplugin.so
	rm -rf "${S}"/opt/insync/PySide2/Qt/qml/QtQuick/Controls.2/Universal/libqtquickcontrols2universalstyleplugin.so
	rm -rf "${S}"/opt/insync/PySide2/Qt/qml/QtQuick/Controls.2/libqtquickcontrols2plugin.so
	rm -rf "${S}"/opt/insync/PySide2/Qt/qml/QtQuick/Particles.2/libparticlesplugin.so
	rm -rf "${S}"/opt/insync/PySide2/Qt/qml/QtQuick/Shapes/libqmlshapesplugin.so
	rm -rf "${S}"/opt/insync/PySide2/Qt/qml/QtQuick/Templates.2/libqtquicktemplates2plugin.so
	rm -rf "${S}"/opt/insync/PySide2/Qt/qml/QtTest/libqmltestplugin.so
